...
- **Hinge Health** — ahead of the other three now (2026-09-04 session). Hero recropped (`heroObjectPosition`, per-case-study override added to `CaseStudyFrontmatter`/`CaseStudyHero` so it doesn't affect other photo heroes). Both `MediaSplit` sections have real copy (Taylor's own account, no more `TODO`), including a Section 2 `Outcome` block. Section 1 (UX Research) has real images via the new `ScrollSplit`: a static AgeBold competitor deep-dive image + the full competitive-analysis table (Nymbl/Sword Health/AgeBold/Balanced) in a scrollable/expandable frame — Taylor picked AgeBold over Nymbl's deep-dive spread for this slot (Nymbl's second panel was too text-dense at MediaSplit's column width); Nymbl's spread + AgeBold's individual panels are earmarked for a future supplementary carousel once Taylor has more assets. Section 2 (North Star Design) still needs real image(s). Two real client quotes have been live throughout (no image dependency).
